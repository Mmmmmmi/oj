...
// CompileAndRunCpp 函数， 由这个函数来完成编译 + 运行 的功能;
//
...
public:
    ...
    static std::string CppSrcPath(const std::string& name) {
    ...
    //3. python 代码文件
    static std::string PythonSrcPath(const std::string& name) {
        return "./temp_files/" + name + ".py";
    }
    static bool CompileAndRunCpp(const Json::Value& req_json, Json::Value& resp_json)
    {
        //1.  根据请求对象生成源代码文件
        ...
        std::string file_name = WriteCppTmpFile(code, req_json["stdin"].asString());
    ...
    static bool RunPython(const Json::Value& req_json, Json::Value& resp_json)
    {
        //1.  根据请求对象生成源代码文件
        if (req_json["code"].empty()) {
            resp_json["error"] = 3;
        ...
        const std::string& code = req_json["code"].asString();
        //通过这个函数完成把代码写到代码文件的过程 
        std::string file_name = WritePythonTmpFile(code, req_json["stdin"].asString());
        
        //2. python 直接运行 
        int signo = RunPython(file_name);
        if (signo) {
            //错误处理
            resp_json["error"] = 2;
            resp_json["reason"] = "Program Exit By Signo: " + std::to_string(signo);
...
private:
    ...
    //2. 给这次请求分配一个文件名
    //  tmp_(时间戳).计数器.cpp    tmp_1550976161.1.cpp
    static std::string WriteCppTmpFile(const std::string& code, const std::string& stdininfo) 
    {
        ...
        std::string file_name = "tmp_" + std::to_string(TimeUtil::TimeStamp())
            + "." +  std::to_string(id);
        FileUtil::Write(CppSrcPath(file_name), code);  
        FileUtil::Write(StdinPath(file_name), stdininfo);
        return file_name;
    }

    static std::string WritePythonTmpFile(const std::string& code, const std::string& stdininfo) 
    {
        //定义一个原子的id变量  使用方法与普通的相同  但是使用算数操作时是原子的
        //原子操作依赖cpu支持
        static std::atomic_int id(0);
        ...
        std::string file_name = "tmp_" + std::to_string(TimeUtil::TimeStamp())
            + "." +  std::to_string(id);
        FileUtil::Write(PythonSrcPath(file_name), code);  
        FileUtil::Write(StdinPath(file_name), stdininfo);
        return file_name;
    }
    ...
    static bool Compile(const std::string file_name)
    {
        ...
        sprintf(command[1], "%s", CppSrcPath(file_name).c_str());  
    ...
    static int RunCpp(const std::string& file_name)
    {
        //1. 创建子进程
        int ret = fork();
        if (ret > 0) {
    ...
    static int RunPython(const std::string& file_name)
    {
        //1. 创建子进程
        int ret = fork();
        if (ret > 0) {
            ...
            dup2(fd_stderror, 2);
            //4. 子进程进行程序替换
            execlp("python", "python", PythonSrcPath(file_name).c_str(), NULL);
            exit(0);
        }
    }
};
